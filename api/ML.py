...
def renamer(data):
    for s in data:  # looping through each items in data
        sorg = s
        s = s.upper()  # chnged into uppercase
        s = s.strip()  # Remove extra spaces around the data
        if(s.find(' ') > 0):
            s = s.replace(" ", '_', 1)
            if(s.find(' ') > 0):
                s = s[:(s.find(' '))]
...
def training():
    ...
    # todo dont delete once we have lots of data for each sub
    del data["SUBJECT_NAME"]
    # todo dont delete once we have lots of data for each type
    del data["EDUCATION_TYPE"]
    # todo dont delete once we have lots of data for each sem
    del data["SEMESTER"]
    # todo dont delete once we have lots of data for each sem
    del data["DUE_DATE"]
    # start deletion
    ...
    reg = RandomForestRegressor(
        n_estimators=60, max_depth=3, max_features='auto')
...
def pred(data2):
    ...
    pkl_filename = "./pickle_model.pkl"
    pkl_filename = os.path.join(os.path.abspath(
        os.path.dirname(__file__)), pkl_filename)
    ...
    pkl_filename2 = "./pickle_scaler.pkl"
    pkl_filename2 = os.path.join(os.path.abspath(
        os.path.dirname(__file__)), pkl_filename2)
